...
angular.module('gosteiclubApp')
  .controller('ReceitasDetailCtrl', function ($scope, $modal, $rootScope,
      $translate, $location, Menu, $http, 
      User, Utils, Product, SessionLanding, Receita, $routeParams) {

        


        var receitaName =  $routeParams.name.replace(/-/g, ' ').trim();


        $http.get('/api/receita?name='+receitaName).then(function(response){

          console.log('$routeParams', $routeParams.name.replace(/-/g, ' ').trim());
